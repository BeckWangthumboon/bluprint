...
const fsMove = (from: string, to: string) =>
  ResultAsync.fromPromise(
    ...
    (error) =>
      new Error(`Unable to move ${from} to ${to}: ${(error as Error).message}`)
  );
...
const fsCheckAccess = (path: string) =>
  ResultAsync.fromPromise(
    ...
    (error) =>
      new Error(`Unable to access ${path}: ${(error as Error).message}`)
  );
...
const fsReadFile = (target: string) =>
  ResultAsync.fromPromise(
    ...
    (error) =>
      new Error(`Unable to read file at ${target}: ${(error as Error).message}`)
  );
...
const fsWriteFile = (target: string, data: string) =>
  ResultAsync.fromPromise(
    ...
    (error) =>
      new Error(
        `Unable to write file at ${target}: ${(error as Error).message}`
      )
  );
