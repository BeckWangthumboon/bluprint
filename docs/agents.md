...
2. The runtime asks `llm/registry.ts` for a `LanguageModel`. The registry:
   - Reads `PROVIDER` from environment variables (defaults to `openrouter`; supports `zai`, `vertex`).
   ...
   - For `vertex`: uses Google Cloud Application Default Credentials (ADC) for authentication:
     ...
     - **Optional**: `GOOGLE_VERTEX_PROJECT` — Google Cloud project ID (defaults to project from credentials)
     - **Optional**: `GOOGLE_VERTEX_LOCATION` — Google Cloud region (defaults to `us-central1`)
